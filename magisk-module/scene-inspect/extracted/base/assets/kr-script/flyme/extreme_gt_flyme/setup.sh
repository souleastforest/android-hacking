...
if [[ -e $chg ]]; then
wired_level="/sys/class/meizu/charger/wired_level"
wired_level2="/sys/class/meizu/charger/wired/wired_level"
wls_level="/sys/class/meizu/wireless/wls_level"
wls_level2="/sys/class/meizu/wireless/wired/wls_level"
if [[ -f $wired_level2 ]]; then
  wired_level=$wired_level2
fi
if [[ -f $wls_level2 ]]; then
  wls_level=$wls_level2
fi
echo '[setting]
...
path = /sys/class/meizu/wireless/reverse_chg_enable
value = 1
...
path = /sys/class/meizu/wireless/tx_fan_mode
value = 0
...
[wireless_tx_model]
path = /sys/class/meizu/wireless/txid
' > $module$chg
fi
